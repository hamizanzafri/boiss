...
class PaymentController extends Controller {
    ...
    public function success(Request $request) {
        ...
        try {
            ...
            if ($order) {
                ...
                // Send receipt email if the user bought a product
    ...
    public function showReceipt($payment_id) {
        try {
            ...
            // Generate receipt HTML and PNG
            $html = view('receipt', compact('payment', 'membership', 'order', 'orderDetails', 'totalPaid'))->render();
            $htmlPath = storage_path('receipt.html');
            file_put_contents($htmlPath, $html);
    
            $process = new Process(['node', base_path('generate-receipt.js')]);
            $process->run();
            ...
            $imagePath = storage_path('receipt.png');
    
            // Send receipt email with attachment
            Mail::to($order->email)->send(new ReceiptMail($order, $imagePath));
    
            // Determine the view to be used based on the order type
            if (strpos($order->type, 'Event') !== false) {
                return redirect()->route('home')->with('message', 'Event Ticket Purchased!');
            } elseif (strpos($order->type, 'Event') !== false && strpos($order->type, 'Product') !== false) {
                return view('both', compact('payment', 'membership', 'order', 'orderDetails', 'totalPaid'));
            } else {
                return view('receipt', compact('payment', 'membership', 'order', 'orderDetails', 'totalPaid'));
            }
